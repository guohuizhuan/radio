...
#define  ADC_TASK_TEMPERATURE_SAMPLE_INDEX   1
#define  ADC_TASK_RADIO_SAMPLE_INDEX         0
    ...
    if (signals.status == osEventSignal) {
        if (signals.value.signals & ADC_COMPLETED_SIGNAL) {
            if (cnt < ADC_SAMPLE_CNT_MAX) {
                ...
                temperature_msg.adc.value = adc_average[ADC_TASK_TEMPERATURE_SAMPLE_INDEX];
                ...
                radio_msg.adc.value = adc_average[ADC_TASK_RADIO_SAMPLE_INDEX];

                if (xQueueSend(radio_msg_q_hdl,&radio_msg,ADC_TASK_PUT_MSG_TIMEOUT) != pdPASS) {
                    log_error("send radio msg err.\r\n");
                }
            }
        }
